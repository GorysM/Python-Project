...
for key in answers:
    if answers[key]:
        for ans in answers[key]:
            f = open(r'F:\OneDrive\Υπολογιστής\New folder\credits.txt', 'a') # Open file with the 'w' parameter so we can write in it 
            f.write(ans+'\n')
    
f.close()

print('Open the file again and print it\n')
f = open(r'F:\OneDrive\Υπολογιστής\New folder\credits.txt', 'r')
print(f.read())
